...
@section('content')
	<div class="row mb-4 px-2">
		...
		<a href="{{ route('vacante.create') }}" class="col col-md-2 btn btn-sm green darken-2 text-white"><i class="fa fa-plus"></i> Nuevo</a>
	</div>
							...
							@foreach ($vacantes as $vac)
								<tr data-card="{{$vac->id}}">
									<td class="align-middle">
										<a href="{{route('vacante.show', $vac->id)}}">
											{{$vac->titulo}}
										</a>
									</td>
									{{-- <td class="align-middle">
									...
									<td class="align-middle">
										<div class="custom-control custom-switch" data-table="vacante" data-campo="activo">
											<input type="checkbox" @if ($vac->activo) checked @endif class="custom-control-input swiToAj" data-id="{{$vac->id}}" id="AswiTo-{{$vac->id}}">
											<label class="custom-control-label" for="AswiTo-{{$vac->id}}"></label>
										</div>
									</td>
									<td class="align-middle">
										<div class="dropdown text-right">
											...
											<div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
												<a class="dropdown-item" href="{{route('vacante.show', $vac->id)}}"><i class="fas fa-info-circle"></i> Ver más</a>
												<a class="dropdown-item" href="{{route('vacante.edit', $vac->id)}}"><i class="far fa-fw fa-edit"></i> Editar</a>
												<button type="button" class="dropdown-item" data-toggle="modal" data-target="#frameModalDel" data-id="{{$vac->id}}"><i class="fas fa-fw fa-trash-alt"></i> Eliminar </button>
											</div>
										</div>
									</td>
								</tr>
			...
			<div class="modal-content">
				<div class="modal-body bg-danger">
					<div class="row d-flex justify-content-center align-items-center">
						...
						<form id="tamdel" action="{{ route('vacante.delete') }}" method="POST" style="display: none;">
								@csrf
